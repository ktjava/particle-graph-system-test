...
void ofApp::setup(){
	  ...
	  ofSetWindowTitle("Particle Graph System Test");
...
void ofApp::draw(){
	  ...
	  for(int i=0; i<NODE_NUMBER-1; ++i){
		  ofDrawLine(particleSet[i]->position.x,particleSet[i]->position.y,particleSet[i+1]->position.x,particleSet[i+1]->position.y);
	  }
	  c.set(0,0,0,16);
  	  ofSetColor(c);
